...
router.get('/', authenticateAccessToken, async (req, res) => {
    try {
        // const orders = await Order.find().populate('products').populate('owner', 'username email');
...
router.put('/:id/accept', authenticateAccessToken, async (req, res) => {
    const { id } = req.params;

    try {
        const order = await Order.findByIdAndUpdate(
            id,
            { status: 'Accepted', isAccepted: true, updatedAt: Date.now() },
            { new: true }
        );
